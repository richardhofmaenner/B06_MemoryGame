...
class GameScreen():
    ...
    def __init__(self):
        ...
        for i in range(16):
            ...
            cardButton = tk.Button(
                self.gameWindow, text=f"Card {i+1}", width=10, height=5, command=self.count_clicks
            )
            cardButton.grid(row=row, column=column)
            cardButtons.append(cardButton)

        startButton = tk.Button(
            self.gameWindow, text="Start Game", **style.buttonStyle,
            command=lambda: print("Start Game"), width=10, height=3
        )
        ...
        self.countButton = tk.Button(
            self.gameWindow, text=f"Click Counts: {self.clicks}", width=10, height=3
        )
